...
function applyOutline(
  ...
  if (kind === 'broken') {
    ...
    inkStroke(ctx, cx, cy, rx * 0.98, ry * 0.98, ink, {
      seed: seed + 5,
  ...
  if (kind === 'double') {
    inkStroke(ctx, cx, cy, rx * 1.04, ry * 1.04, ink, {
      seed,
    ...
    inkStroke(ctx, cx, cy, rx * 0.9, ry * 0.88, mixColor(ink, pal.body, 0.35), {
      seed: seed + 2,
  ...
  if (kind === 'layers') {
    for (let i = 0; i < 3; i++) {
      const s = 1 - i * 0.16
      inkStroke(ctx, cx, cy + ry * 0.02 * i, rx * s, ry * s, ink, {
        seed: seed + i,
...
function fillSilhouette(
  ...
  ctx.fill(wobbleBlob(r * 0.1, r * 0.28, r * 0.45, r * 0.55, seed, { n: 7, wobble: 0.2 }))
  ctx.globalAlpha = 0.22
  ctx.fillStyle = mixColor(pal.body, '#fff6e8', 0.4)
  ctx.fill(wobbleBlob(-r * 0.18, -r * 0.22, r * 0.28, r * 0.2, seed + 2, { n: 6, wobble: 0.22 }))
  ctx.restore()
}
...
function paintPotato(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, 0.02 * r, r * 1.08, r * 0.86, pal.body, { seed, shade: pal.shade, n: 11, wobble: 0.22 })
  wash(ctx, -r * 0.52, r * 0.18, r * 0.48, r * 0.52, pal.body, { seed: seed + 2, shade: pal.shade, n: 8, wobble: 0.2, passes: 3 })
  wash(ctx, r * 0.5, -r * 0.12, r * 0.42, r * 0.48, pal.body, { seed: seed + 4, shade: pal.shade, n: 8, wobble: 0.2, passes: 3 })
  for (let i = 0; i < 5; i++) {
...
function paintTurnip(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  ...
  ctx.save()
  ctx.clip(path)
  wash(ctx, 0, r * 0.48, r * 0.95, r * 0.52, pal.shade, { seed: seed + 3, n: 8, wobble: 0.16, passes: 3 })
  ctx.restore()
...
function paintPumpkin(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  ...
  for (let i = 0; i < lobes.length; i++) {
    const lobe = lobes[i]
    if (!lobe) continue
    wash(ctx, r * lobe[0], r * 0.08, r * lobe[1], r * 0.92, pal.body, {
      seed: seed + i,
      shade: pal.shade,
      n: 7,
      wobble: 0.08,
...
function paintRadish(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  ...
  ctx.closePath()
  ctx.clip()
  wash(ctx, 0, r * 0.72, r * 0.3, r * 0.42, '#f2ead8', { seed: seed + 3, shade: '#d8c8b0', n: 6, wobble: 0.16, passes: 2 })
  ctx.restore()
...
function paintEggplant(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, r * 0.22, r * 0.78, r * 1.12, pal.body, { seed, shade: pal.shade, n: 9, wobble: 0.1 })
  wash(ctx, 0, -r * 0.35, r * 0.52, r * 0.55, pal.body, { seed: seed + 2, shade: pal.shade, n: 7, wobble: 0.1, passes: 3 })
  ctx.globalAlpha = 0.5
...
function paintPea(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, 0, r * 0.78, r * 0.78, pal.body, { seed, shade: pal.shade, n: 8, wobble: 0.1 })
  ctx.globalAlpha = 0.5
...
function paintSprout(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, r * 0.48, r * 0.34, r * 0.42, mixColor(pal.body, '#e8f0c8', 0.3), { seed, shade: pal.shade, n: 7, wobble: 0.16 })
  wash(ctx, 0, r * 0.62, r * 0.4, r * 0.32, pal.body, { seed: seed + 6, shade: pal.shade, n: 8, wobble: 0.16 })
  applyOutline(ctx, 0, r * 0.52, r * 0.42, r * 0.48, pal, seed, nm, 'sprout')
  inkStroke(ctx, 0, r * 0.52, r * 0.42, r * 0.48, outlineInk(pal, 'sprout'), {
    seed,
...
function paintGarlic(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  ...
  for (let i = 0; i < cloves.length; i++) {
    const c = cloves[i]
    if (!c) continue
    wash(ctx, r * c[0], r * c[1], r * c[2], r * c[3], pal.body, {
      seed: seed + i,
      shade: pal.shade,
      n: 7,
      wobble: 0.14,
...
function paintBeet(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, 0, r * 0.95, r * 0.88, pal.body, { seed, shade: pal.shade, n: 8, wobble: 0.12 })
  wash(ctx, 0, r * 0.72, r * 0.28, r * 0.32, pal.shade, { seed: seed + 2, n: 6, wobble: 0.18, passes: 2 })
...
function paintCorn(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, 0, r * 0.48, r * 1.08, pal.body, { seed: seed + 4, shade: pal.shade, n: 7, wobble: 0.07 })
...
function paintBroccoli(ctx: CanvasRenderingContext2D, pal: Palette, r: number, seed: number, nm: boolean): void {
  wash(ctx, 0, r * 0.5, r * 0.26, r * 0.52, '#c4a06a', { seed, shade: '#8a6a38', n: 6, wobble: 0.12, passes: 3 })
...
function paintSpeciesMarks(ctx: CanvasRenderingContext2D, species: Species, pal: Palette, r: number, seed: number): void {
  switch (species) {
    ...
    case 'corn':
      ...
      break
    case 'radish':
      wash(ctx, 0, r * 0.72, r * 0.22, r * 0.28, '#f2ead8', { seed: seed + 3, shade: '#d8c8b0', n: 6, wobble: 0.16, passes: 2 })
      break
    default:
      break
  }
...
function paintMouth(
  ...
  if (kind === 'o' || kind === 'tiny' || kind === 'nub' || kind === 'pout') {
    ...
    if (kind === 'pout' && open < 0.2) {
      ...
      ctx.ellipse(0, h * 0.1, w * 0.28 * scale + open * w * 0.12, h, 0, 0, TAU)
...
function paintOneEye(
  ...
  if (nm) {
    ...
    if (kind === 'carved') {
      ...
      ctx.closePath()
    } else if (kind === 'slit' || kind === 'crescent') {
      ctx.ellipse(0, 0, rs * 1.3, rs * 0.55, index === 0 ? 0.45 : -0.45, 0, TAU)
    } else {
      ctx.arc(0, 0, rs * 1.5 * flare, 0, TAU)
    }
    ctx.fill()
    ctx.restore()
  }
  ...
  if (kind === 'crescent') {
    ctx.strokeStyle = nm ? glow : pal.ink
    ctx.lineWidth = rs * (nm ? 0.38 : 0.28)
    ctx.lineCap = 'round'
    ctx.beginPath()
  ...
  const rx =
    kind === 'saucer' ? rs * 0.7 : kind === 'almond' ? rs * 0.52 : kind === 'kernel' ? rs * 0.4 : kind === 'lopsided' ? rs * (index === 0 ? 0.4 : 0.55) : rs * 0.48
  const ry =
    kind === 'saucer' ? rs * 0.72 : kind === 'almond' ? rs * 0.38 : kind === 'kernel' ? rs * 0.48 : kind === 'hooded' ? rs * 0.5 : rs * 0.55
  const rot = kind === 'almond' ? (index === 0 ? 0.18 : -0.18) : kind === 'kernel' ? 0.1 : 0

  ctx.fillStyle = nm ? glow : scleraOf(species)
  ctx.beginPath()
  if (kind === 'kernel') {
  ...
  if (nm) {
    ...
    ctx.globalAlpha = 0.85
    ctx.beginPath()
    ctx.arc(-rs * 0.18, -rs * 0.18, rs * 0.16, 0, TAU)
    ctx.fill()
    ctx.restore()
    return
  }

  const px = clamp(lookX, -1, 1) * rx * 0.35
  const py = clamp(lookY, -1, 1) * ry * 0.28
  ctx.fillStyle = pal.eye
  ctx.beginPath()
  ctx.ellipse(px, py, rx * (kind === 'stare' ? 0.55 : 0.42), ry * (kind === 'stare' ? 0.55 : 0.42), 0, 0, TAU)
  ctx.fill()
  ctx.fillStyle = 'rgba(255,255,255,0.9)'
  ctx.beginPath()
  ctx.arc(px - rx * 0.18, py - ry * 0.2, rs * 0.08, 0, TAU)
  ctx.fill()
  ctx.strokeStyle = pal.ink
...
function paintEyePair(
  ...
  for (let i = 0; i < eyes.length; i++) {
    ...
    const rs = rig.eyeS * (scream >= 0.4 && kind !== 'dot' ? 1.12 : 1)
...
function paintSpeciesMotion(ctx: CanvasRenderingContext2D, o: LiveOpts, pal: Palette): void {
  ...
  if (species === 'carrot' || species === 'turnip' || species === 'radish' || species === 'beet' || species === 'potato') {
    const n = species === 'carrot' ? 5 : species === 'beet' ? 4 : species === 'potato' ? 2 : 3
  ...
  if (species === 'sprout') {
    ...
    for (let i = 0; i < leaves.length; i++) {
      ...
      ctx.rotate(leaf[2] * f)
      wash(ctx, 0, 0, r * (0.48 - i * 0.04), r * 0.2, i === 0 ? acc : pal.body, {
        seed: 20 + i,
        shade: pal.shade,
        n: 7,
        wobble: 0.25,
  ...
  if (species === 'cabbage') {
    for (let i = 0; i < 3; i++) {
      ...
      ctx.rotate(a * 0.4 * f)
      ctx.globalAlpha = 0.85
      wash(ctx, 0, 0, r * 0.55, r * 0.28, i % 2 ? acc : pal.body, {
        seed: 30 + i,
        shade: pal.shade,
        n: 7,
        wobble: 0.22,
  ...
  if (species === 'corn') {
    ...
    wash(ctx, 0, 0, r * 0.32, r * 0.95, acc, { seed: 40, shade: darken(acc, 0.2), n: 7, wobble: 0.18, passes: 2 })
    ctx.restore()
    ctx.save()
    ctx.translate(r * 0.22, r * 0.05)
    ctx.rotate(0.3 - rustle)
    wash(ctx, 0, 0, r * 0.3, r * 0.9, acc, { seed: 42, n: 7, wobble: 0.18, passes: 2 })
    ctx.restore()
    return
  }
...
export function paintLiveFeatures(ctx: CanvasRenderingContext2D, o: LiveOpts): void {
  ...
  paintBrows(ctx, o.species, pal, rig, f)
  paintEyePair(ctx, pal, o.species, rig, f, o.lookX, o.lookY, o.blink, o.scream, o.form, o.t)
  paintMouth(ctx, pal, rig.mouth.x, rig.mouth.y, rig.mouthW, o.mouth, o.form, f, o.species)
